...
class BaseASR:
    def __init__(self, opt, parent: BaseReal = None):
        ...
        # 流式TTS存在轻微网络抖动，过短超时会频繁回退到静音帧
        self.frame_wait_timeout = max(0.04, (self.chunk / self.sample_rate) * 2)
        self.queue = Queue()
    ...
    def get_audio_frame(self):
        try:
            frame, eventpoint = self.queue.get(block=True, timeout=self.frame_wait_timeout)
            type = 0
